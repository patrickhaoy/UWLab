...
def initial_final_interpolate_fn(env: ManagerBasedRLEnv, env_id, data, initial_value, final_value, difficulty_term_str):
    """Interpolate between initial and final values using difficulty_frac from a DifficultyScheduler.

    Supports arbitrarily nested structures of lists/tuples. Scalars (int/float) are interpolated at the leaves.
    """
    difficulty_term: AssemblyDifficultyScheduler = getattr(env.curriculum_manager.cfg, difficulty_term_str).func
...
class AssemblyDifficultyScheduler(ManagerTermBase):
    """Adaptive difficulty scheduler driven by ProgressContext success.

    Tracks per-environment difficulty levels and adjusts them based on assembly task success.
    Difficulty increases when ProgressContext reports success, and decreases otherwise
    (unless promotion_only is set). The normalized average difficulty across environments
    is exposed as difficulty_frac for use in curriculum interpolation (e.g. gravity ramp,
    partial assembly sampling range).
    """
    ...
    def __call__(
        ...
        progress_context = env.reward_manager.get_term_cfg("progress_context").func
        success = progress_context.success[env_ids]
